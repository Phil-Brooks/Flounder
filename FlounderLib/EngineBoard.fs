...
type EngineBoard =
    ...
    new(fen) as this = 
        ...
        } then RepHist.Reset();NNUEb.AccIndex<-0;NNUEb.ResetAccumulator(this.Brd,0);NNUEb.ResetAccumulator(this.Brd,1)
    ...
    member this.IsRepetition() = 
        RepHist.Count(this.Brd.ZobristHash) > 1
    member this.GuiMove(from:int, mto:int, promotion:int) =
        let moveList = MoveList.ForSq(&this.Brd, from)
        ...
        RepHist.Append(this.Brd.ZobristHash)
