...
module BitBoardMap =
    let FromFields(isWtm,pieces,squares,white,black,both,whiteKCastle,whiteQCastle,blackKCastle,blackQCastle,enPassantTarget,zobristHash) =
        {
            IsWtm = isWtm
            Pieces = pieces
            Squares = squares
    ...
    let Move(map:byref<BitBoardMap>, from:int, mto:int) =
