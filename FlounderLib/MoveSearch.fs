...
type MoveSearch =
    ...
    member this.QSearch(isPvNode:bool, plyFromRoot:int, depth:int, ialpha:int, beta:int) =
            ...
            if ans.IsSome then ans.Value
                ...
                while (keepgoing && i < moveCount) do
                    ...
                    if (seeEval > beta) then 
                        ...
                        let evaluation = -this.QSearch(isPvNode, nextPlyFromRoot, nextDepth, -beta, -alpha)
                        // Undo the move.
                    ...
                    i <- i+1
                bestEvaluation
    member this.AbSearch(isPvNode:bool, plyFromRoot:int, idepth:int, ialpha:int, ibeta:int) =
        let mutable alpha = ialpha
        ...
        if idepth <= 0 then ans <- this.QSearch(isPvNode, plyFromRoot, 15, alpha, beta)|>Some
        else
            ...
            if ans.IsSome then 
                ...
                if not isPvNode && not inCheck then
                    ...
                    if idepth < 7 && Math.Abs(beta) < 99999999 && positionalEvaluation - 67 * idepth + 76 * improvingInt >= beta then
                        ...
                        ans <- this.QSearch(false, plyFromRoot, 15, alpha, beta)|>Some
                    elif not rootNode && idepth > 2 then
                        let evaluation = this.NullMovePrune(nextPlyFromRoot,idepth,beta)
                ...
                if ans.IsSome then 
                    ...
                    if moveCount = 0 then
                            ...
                            if lmpTest then keepgoing <- false
                                ...
                                let mutable evaluation = 
                                    if i = 0 then
                                        ...
                                        // safe.
                                        -this.AbSearch(isPvNode, nextPlyFromRoot, nextDepth, -beta, -alpha)
                                    else 
                                        alpha + 1
                                //Principle Variation Search
                                if i > 0 && evaluation > alpha then
                                    this.PvSearch(&evaluation,nextPlyFromRoot,nextDepth,alpha,beta)
                                // Undo the move.
                                EngBoard.UndoMove(&rv)
                                if not (this.HandleEvaluation(evaluation, move, &bestEvaluation,&bestMoveSoFar,isPvNode,plyFromRoot,&alpha,beta,&transpositionTableEntryType)) then
                                    if quietMove then
                                        this.DoQuiet(plyFromRoot,move,depth,quietMoveCounter,moveList,i)
                        ...
                        let mutable entry = {Hash=Brd.ZobristHash;Type=transpositionTableEntryType;BestMove=bestMoveSoFar;Depth=depth}
                        TranTable.InsertEntry(Brd.ZobristHash, &entry)

                        bestEvaluation
    member this.AspirationSearch(depth:int, previousEvaluation:int) =
        ...
        while notfound do
            ...
            bestEvaluation <- this.AbSearch(true, 0, depth, alpha, beta)
            //Modify Window
    ...
    member this.HandleEvaluationQ(evaluation:int, bestEvaluation:byref<int>, alpha:byref<int>, beta:int) =
        ...
        // with the new evaluation.
            ...
            if evaluation <= alpha then true
                ...
                evaluation < beta
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    member this.NullMovePrune(nextPlyFromRoot:int, idepth:int, beta:int) =        
        ...
        // QSearch).
        let evaluation = -this.AbSearch(false, nextPlyFromRoot, reducedDepth, -beta, -beta + 1);
        // Undo the null move so we can get back to original state of the board.
        EngBoard.UndoNullMove(rv)
        evaluation
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    member this.PvSearch(evaluation:byref<int>, nextPlyFromRoot:int, nextDepth:int, alpha:int, beta:int) =
        ...
        evaluation <- -this.AbSearch(false, nextPlyFromRoot, nextDepth, -alpha - 1, -alpha)
        if (evaluation > alpha && evaluation < beta) then
            ...
            evaluation <- -this.AbSearch(true, nextPlyFromRoot, nextDepth, -beta, -alpha)
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
