...
type MoveSearch(board:EngineBoard, table:MoveTranspositionTable, timeControl:TimeControl) =
    ...
    member this.QSearch(node:NodeType, board:EngineBoard, plyFromRoot:int, depth:int, ialpha:int, beta:int) =
        ...
        if (node = NonPvNode) then
            let storedEntry = Table.Value.[board.IBoard.ZobristHash]
            if (storedEntry.ZobristHash = board.IBoard.ZobristHash &&
        ...
        if ans.IsSome then 
            ...
            let mutable earlyEval = Evaluation.Relative(board.IBoard)
            ...
            if ans.IsSome then ans.Value
                ...
                let moveCount = moveList.QSearchMoveGeneration(board.IBoard, SearchedMove.Default)
    ...
    member this.AbSearch(node:NodeType, board:EngineBoard, plyFromRoot:int, idepth:int, ialpha:int, ibeta:int) =
        ...
        if (depth <= 0) then ans <- this.QSearch(node, board, plyFromRoot, 15, alpha, beta)|>Some
            ...
            if (notRootNode) then
                ...
                if (board.IsRepetition()) then ans <- 0|>Some
                    ...
                    let allPiecesCount = board.IBoard.All().Count
                    // If only the kings are left, it's a draw.
                    if (allPiecesCount = 2) then ans <- 0|>Some
                    else
                        let knightLeft = board.IBoard.All(Piece.Knight, PieceColor.White).ToBool() || board.IBoard.All(Piece.Knight, PieceColor.Black).ToBool()
                        // If only the kings and one knight is left, it's a draw.
                        if (allPiecesCount = 3 && knightLeft) then ans <- 0|>Some
                        else
                            let bishopLeft = board.IBoard.All(Piece.Bishop, PieceColor.White).ToBool() || board.IBoard.All(Piece.Bishop, PieceColor.Black).ToBool()
        ...
        if ans.IsSome then 
            ...
            if (valid && storedEntry.ZobristHash = board.IBoard.ZobristHash) then
            ...
            if ans.IsSome then 
                ...
                let oppositeColor = PieceColor.OppositeColor(board.IBoard.ColorToMove)
                let kingSq = board.IBoard.KingLoc(board.IBoard.ColorToMove).ToSq()
                let mutable inCheck = MoveList.UnderAttack(board.IBoard, kingSq, oppositeColor)
                ...
                let positionalEvaluation = if transpositionHit then transpositionMove.Evaluation else Evaluation.Relative(board.IBoard)
                ...
                if ans.IsSome then 
                    ...
                    let mutable moveSpan = new Span<OrderedMoveEntry>(moveSpanarr)
                    let moveList = OrderedMoveList(moveSpan, plyFromRoot, KillerMoveTable, HistoryTable)
                    let moveCount = moveList.NormalMoveGeneration(board.IBoard, transpositionMove)
                    if (moveCount = 0) then
                    ...
                    if ans.IsSome then 
                        ...
                        while (i < moveCount && keepgoing) do
                            ...
                            let quietMove = not (board.IBoard.All(oppositeColor).[move.To])
                            ...
                            //Late Move Pruning
                                ...
                                if not (handleEvaluation(evaluation, move, quietMove)) then
                                    if (quietMove) then
                                        ...
                                        HistoryTable.[board.PieceOnly(move.From), board.IBoard.ColorToMove, move.To] <- HistoryTable.[board.PieceOnly(move.From), board.IBoard.ColorToMove, move.To] + historyBonus
                                        ...
                                        while (j < quietMoveCounter) do
                                            ...
                                            HistoryTable.[board.PieceOnly(otherMove.From), board.IBoard.ColorToMove, otherMove.To] <- HistoryTable.[board.PieceOnly(otherMove.From), board.IBoard.ColorToMove, otherMove.To] - historyBonus
                                            j <- j+1
                        ...
                        let mutable entry = MoveTranspositionTableEntry(board.IBoard.ZobristHash, transpositionTableEntryType, bestMove, depth)
                        Table.Value.InsertEntry(board.IBoard.ZobristHash, &entry)
