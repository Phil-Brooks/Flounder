...
module MoveList =
    ...
    let LegalPawnMoveCaptures(iswtm:bool, board:byref<BoardRec>, from:int, hv:uint64, d:uint64, c:uint64) =
        ...
        if Bits.IsSet(hv, from) then
            ...
            let opposite = if iswtm then board.Black else board.White
            ...
            if Bits.IsSet(d, from) then
                ...
                if (epPieceSq <> Na) then
                    ...
                    BitBoardMap.Empty(&board, from)
                    BitBoardMap.Empty(&board, epPieceSq)
                    BitBoardMap.InsertPiece(&board, color, board.EnPassantTarget)
                    ...
                    BitBoardMap.InsertPiece(&board, color, from)
                    BitBoardMap.InsertPiece(&board, oppositeColor, epPieceSq)
                    BitBoardMap.Empty(&board, board.EnPassantTarget)
                    ...
                    moves,promotion                
    let LegalPawnMoves(color:int, board:byref<BoardRec>, from:int, hv:uint64, d:uint64, c:uint64) =
        let mutable moves = 0UL
        let oppositeColor = color^^^1
        let colBoard = if color = White then board.White else board.Black
        let opposite = if color = White then board.Black else board.White
        let mutable epPieceSq = Na
        //Promotion Flag
        // If we're at rank 7 for white or rank 1 for black, we should set the promotion flag to true.
        // It is important to set it earlier rather than later, because if there is a diagonal pin capture
       ...
       // Make sure attacks are only on opposite pieces (and not on empty squares or squares occupied by
        ...
        if Bits.IsSet(d, from) then
            ...
            // Push pawn once.
            pushes <- pushes ||| (if color = 0 then Bits.FromSq(from) >>> 8 else Bits.FromSq(from) <<< 8) &&& ~~~board.Both
            ...
            if Bits.IsSet(hv, from) then
                ...
                if epPieceSq <> Na then
                    ...
                    moves,promotion
    let LegalRookMoves(color:int, board:BoardRec, from:int, hv:uint64, d:uint64, c:uint64) =
        let mutable moves = 0UL
        let colBoard = if color = White then board.White else board.Black
        ...
        if Bits.IsSet(d, from) then moves
            ...
            moves
    let LegalKnightMoves(color:int, board:BoardRec, from:int, hv:uint64, d:uint64, c:uint64) =
        let mutable moves = 0UL
        let colBoard = if color = White then board.White else board.Black
    ...
    let LegalBishopMoves(color:int, board:BoardRec, from:int, hv:uint64, d:uint64, c:uint64) =
        ...
        if Bits.IsSet(hv, from) then moves
            ...
            moves
    let LegalQueenMoves(color:int, board:BoardRec, from:int, hv:uint64, d:uint64, c:uint64) =
        LegalRookMoves(color,board,from,hv,d,c) 
        ||| LegalBishopMoves(color,board,from,hv,d,c)
    let LegalKingMoves(color:int, board:byref<BoardRec>, from:int, hv:uint64, d:uint64, c:uint64) =
        let mutable moves = 0UL
        let colBoard = if color = White then board.White else board.Black
        // Normal
        ...
        if kingMoves = 0UL then 
            ...
            for move in movearr do
                if (UnderAttack(board, move, ioppositeColor)) then Bits.PopBit(&kingMoves, move)
            BitBoardMap.InsertPiece(&board, (if color=0 then WhiteKing else BlackKing), from)
            ...
            if (UnderAttack(board, from, ioppositeColor)) then moves
                ...
                // Get castling rights.
                let q = if color = White then board.WhiteQCastle else board.BlackQCastle
                let k = if color = White then board.WhiteKCastle else board.BlackKCastle
...
type MoveList =
    struct
        ...
        val Count:int
        val Promotion:bool
        new(board:BoardRec, from:int, horizontalVertical:uint64, diagonal:uint64, checks:uint64, moves:uint64, promotion:bool) =
            {
                Board = board
                From = from
        ...
        new(board:byref<BoardRec>, from:int) =
        ...
        new(board:byref<BoardRec>, from:int, piece:int, color:int, horizontalVertical:uint64, diagonal:uint64, checks:uint64, doubleChecked:bool) =
        ...
        new(board:byref<BoardRec>, from:int, piece:int, color:int, horizontalVertical:uint64, diagonal:uint64, checks:uint64) =
            let mutable promotion = false
            let moves =
                if piece=Pawn then 
                    let m,p = MoveList.LegalPawnMoves(color,&board,from,horizontalVertical,diagonal,checks)
                    ...
                    failwith "Cannot generate move for empty piece"
            new MoveList(board, from, horizontalVertical, diagonal, checks, moves, promotion)
        new(board:byref<BoardRec>, from:int, horizontalVertical:uint64, diagonal:uint64, checks:uint64) =
            //this can only be used when processing pawn captiures
            let moves,promotion = MoveList.LegalPawnMoveCaptures(board.IsWtm,&board,from,horizontalVertical,diagonal,checks)
            new MoveList(board, from, horizontalVertical, diagonal, checks, moves, promotion)
