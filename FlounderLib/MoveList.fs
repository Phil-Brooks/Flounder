...
module MoveList =
    ...
    let UnderAttack(board:Board, sq:int, by:int) =
        ...
        if (pawnAttack.Internal &&& board.All(Pawn, by)) <> 0UL then true
        ...
        // knights exist on the squares in the mask. If so, our square can be attacked by knights.
        elif (AttackTable.KnightMoves.[sq].Internal &&& board.All(Knight, by)) <> 0UL then true
        else
            ...
            let mutable mIndex = BlackMagicBitBoardFactory.GetMagicIndex(Rook, BitBoard(occupied), sq)
            if (AttackTable.SlidingMoves.[mIndex].Internal &&& (queen ||| board.All(Rook, by))) <> 0UL then true
            else
                ...
                // or queen.
                mIndex <- BlackMagicBitBoardFactory.GetMagicIndex(Bishop, BitBoard(occupied), sq)
                if (AttackTable.SlidingMoves.[mIndex].Internal &&& (queen ||| board.All(Bishop, by))) <> 0UL then true
                else
                    ...
                    // Otherwise, this square is completely safe from all pieces.
                     (AttackTable.KingMoves.[sq].Internal &&& board.All(King, by)) <> 0UL
    let CheckBitBoard(board:Board, sq:int, by:int) =
        let mutable count = 0
        let mutable checks = 0UL
        // First we generate a pawn check.
        let pawnAttack = if by = 0 then AttackTable.BlackPawnAttacks.[sq].Internal else AttackTable.WhitePawnAttacks.[sq].Internal
        let pawnCheck = pawnAttack &&& board.All(Pawn, by)
        // Next, we generate a knight check.
        let knightCheck = AttackTable.KnightMoves.[sq].Internal &&& board.All(Knight, by)
        ...
        let mutable mIndex = BlackMagicBitBoardFactory.GetMagicIndex(Rook, BitBoard(occupied), sq)
        let rookQueenCheck = AttackTable.SlidingMoves.[mIndex].Internal &&& (queen ||| board.All(Rook, by))
        // Next, we generate a bishop or queen (diagonal only) check.
        mIndex <- BlackMagicBitBoardFactory.GetMagicIndex(Bishop, BitBoard(occupied), sq)
        let bishopQueenCheck = AttackTable.SlidingMoves.[mIndex].Internal &&& (queen ||| board.All(Bishop, by))
        ...
        if rookQueenCheck <> 0UL then
            ...
            checks <- checks ||| UtilityTable.Between.[sq].[int(rqSq)].Internal ||| BitBoard.FromSq(rqSq).Internal
            count<-count+1
        ...
        if bishopQueenCheck <> 0UL then
            ...
            checks <- checks ||| UtilityTable.Between.[sq].[int(bqSq)].Internal ||| BitBoard.FromSq(bqSq).Internal
            count<-count+1
    ...
    let PinBitBoards(board:Board, sq:int, us:int, by:int) =
        ...
        let mutable mIndex = BlackMagicBitBoardFactory.GetMagicIndex(Rook, BitBoard(byBoard), sq)
        let rookQueenCheck = AttackTable.SlidingMoves.[mIndex].Internal &&& (queen ||| board.All(Rook, by))
        // Next, we generate all bishop / queen (diagonal only) attacks.
        mIndex <- BlackMagicBitBoardFactory.GetMagicIndex(Bishop, BitBoard(byBoard), sq)
        let bishopQueenCheck = AttackTable.SlidingMoves.[mIndex].Internal &&& (queen ||| board.All(Bishop, by))
        ...
        let dorqSq rqSq =
            let possiblePin = UtilityTable.Between.[sq].[rqSq].Internal ||| BitBoard.FromSq(rqSq).Internal
        ...
        let dobqSq bqSq =
            let possiblePin = UtilityTable.Between.[sq].[bqSq].Internal ||| BitBoard.FromSq(bqSq).Internal
    ...
    let LegalPawnMoveCaptures(color:int, board:Board, from:int, hv:BitBoard, d:BitBoard, c:BitBoard) =
        ...
        if hv.[from] then
            ...
            // our pieces).
            moves <- moves ||| (attack &&& BitBoard(opposite) &&& c)
            if (d.[from]) then
    ...
    let LegalPawnMoves(color:int, board:Board, from:int, hv:BitBoard, d:BitBoard, c:BitBoard) =
       ...
       // Make sure attacks are only on opposite pieces (and not on empty squares or squares occupied by
        ...
        if (d.[from]) then
            ...
            // Push pawn once.
            pushes <- pushes ||| (if color = 0 then BitBoard.FromSq(from) >>> 8 else BitBoard.FromSq(from) <<< 8) &&& BitBoard(board.All(2))
            ...
            pushes <- pushes &&& ~~~(BitBoard(opposite)) &&& ~~~(BitBoard(board.All(color)))
    ...
    let LegalRookMoves(color:int, board:Board, from:int, hv:BitBoard, d:BitBoard, c:BitBoard) =
        ...
        if d.[from] then moves
            ...
            let mIndex = BlackMagicBitBoardFactory.GetMagicIndex(Rook, ~~~(BitBoard(board.All(2))), from)
            moves <- moves ||| AttackTable.SlidingMoves.[mIndex] &&& ~~~(BitBoard(board.All(color))) &&& c
    ...
    let LegalKnightMoves(color:int, board:Board, from:int, hv:BitBoard, d:BitBoard, c:BitBoard) =
        ...
        if hv.[from] || d.[from] then moves
            ...
            moves <- moves ||| AttackTable.KnightMoves.[from] &&& ~~~(BitBoard(board.All(color))) &&& c
            moves
    let LegalBishopMoves(color:int, board:Board, from:int, hv:BitBoard, d:BitBoard, c:BitBoard) =
        ...
        if (hv.[from]) then moves
            ...
            // Calculate pseudo-legal moves within check board.
            let mIndex = BlackMagicBitBoardFactory.GetMagicIndex(Bishop, ~~~(BitBoard(board.All(2))), from)
            moves <- moves ||| AttackTable.SlidingMoves.[mIndex] &&& ~~~(BitBoard(board.All(color))) &&& c
    ...
    let LegalKingMoves(color:int, board:Board, from:int, hv:BitBoard, d:BitBoard, c:BitBoard) =
        ...
        kingMoves <- kingMoves &&& ~~~(BitBoard(board.All(color)))
...
type MoveList =
    struct
        ...
        new(board:Board, from:int) =
            ...
            new MoveList(board, from, piece, icolor, BitBoard(horizontalVertical), BitBoard(diagonal), BitBoard(checks), doubleChecked)
