...
type MoveList(board:Board1, from:Square, horizontalVertical:BitBoard, diagonal:BitBoard, checks:BitBoard, moves:BitBoard, promotion:bool) =
    ...
    static member UnderAttack(board:Board1, sq:Square, by:PieceColor) =
        ...
        let pawnAttack = if by = PieceColor.White then AttackTable1.BlackPawnAttacks.AA(s) else AttackTable1.WhitePawnAttacks.AA(s)
        ...
        // knights exist on the squares in the mask. If so, our square can be attacked by knights.
        elif (AttackTable1.KnightMoves.AA(s) &&& board.All(Piece.Knight, by)).ToBool() then true
        else
            ...
            if (AttackTable.SlidingMoves.AA(mIndex) &&& (queen ||| board.All(Piece.Rook, by))).ToBool() then true
            else
                ...
                if (AttackTable.SlidingMoves.AA(mIndex) &&& (queen ||| board.All(Piece.Bishop, by))).ToBool() then true
                else
                    ...
                    // Otherwise, this square is completely safe from all pieces.
                     (AttackTable1.KingMoves.AA(s) &&& board.All(Piece.King, by)).ToBool()
    static member CheckBitBoard(board:Board1, sq:Square, by:PieceColor) =
        ...
        let knightCheck = AttackTable1.KnightMoves.AA(s) &&& board.All(Piece.Knight, by)
        ...
        let mutable mIndex = BlackMagicBitBoardFactory.GetMagicIndex(Piece.Rook, occupied, sq)
        let rookQueenCheck = AttackTable.SlidingMoves.AA(mIndex) &&& (queen ||| board.All(Piece.Rook, by))
        // Next, we generate a bishop or queen (diagonal only) check.
        mIndex <- BlackMagicBitBoardFactory.GetMagicIndex(Piece.Bishop, occupied, sq)
        let bishopQueenCheck = AttackTable.SlidingMoves.AA(mIndex) &&& (queen ||| board.All(Piece.Bishop, by))
        ...
        if rookQueenCheck.ToBool() then
            ...
            checks <- checks ||| UtilityTable1.Between.AA(s).[int(rqSq)] ||| BitBoard.FromSq(rqSq)
            count<-count+1
        ...
        if bishopQueenCheck.ToBool() then
            ...
            checks <- checks ||| UtilityTable1.Between.AA(s).[int(bqSq)] ||| BitBoard.FromSq(bqSq)
            count<-count+1
    ...
    static member PinBitBoards(board:Board1, sq:Square, us:PieceColor, by:PieceColor) =
        ...
        while (rookQueenIterator.MoveNext()) do
            let rqS = int(rqSq)
            let possiblePin = UtilityTable1.Between.AA(s).[rqS] ||| BitBoard.FromSq(rqSq)
        ...
        while (bishopQueenIterator.MoveNext()) do
            let bqS = int(bqSq)
            let possiblePin = UtilityTable1.Between.AA(s).[bqS] ||| BitBoard.FromSq(bqSq)
    ...
    static member LegalPawnMoveCaptures(color:PieceColor, board:Board1, from:Square, hv:BitBoard, d:BitBoard, c:BitBoard) =
        ...
        if (hv.[from]) then
            ...
            // Attack Moves.
            let attack = if color = PieceColor.White then AttackTable1.WhitePawnAttacks.AA(int(from)) else AttackTable1.BlackPawnAttacks.AA(int(from))
    ...
    static member LegalRookMoves(color:PieceColor, board:Board1, from:Square, hv:BitBoard, d:BitBoard, c:BitBoard) =
        ...
        if (d.[from]) then moves
            ...
            moves <- moves ||| AttackTable.SlidingMoves.AA(mIndex) &&& ~~~(board.All(color)) &&& c
    ...
    static member LegalKnightMoves(color:PieceColor, board:Board1, from:Square, hv:BitBoard, d:BitBoard, c:BitBoard) =
        ...
        if (hv.[from] || d.[from]) then moves
            ...
            moves <- moves ||| AttackTable1.KnightMoves.AA(int(from)) &&& ~~~(board.All(color)) &&& c
            moves
    ...
    static member LegalKingMoves(color:PieceColor, board:Board1, from:Square, hv:BitBoard, d:BitBoard, c:BitBoard) =
        let mutable moves = BitBoard.Default
        // Normal
        let mutable kingMoves = AttackTable1.KingMoves.AA(int(from))
